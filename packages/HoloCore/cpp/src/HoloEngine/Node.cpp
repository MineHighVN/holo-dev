    ...
    for (const auto& child_cp : this->children) {
        if (std::holds_alternative<std::string>(child_cp)) {
            ...
            for (std::string text : child_split) {
                ...
                Vec2f text_size = this->calcTextSize(text);

                this->drawText(text, Vec2f{ drawPosX, drawPosY });
...
void Node::drawLine(Vec2f begin, Vec2f end, Color col, float thickness) {
    // window->draw_list->AddLine(ImVec2{ x1, y1 }, ImVec2{ x2, y2 }, col, thickness);
}

void Node::drawText(std::string text, Vec2f position) {
    // window->draw_list->AddText(ImVec2{x1, x2}, IM_COL32(0, 0, 0, 255), text.c_str());
    this->graphics->drawText(text, position);
}

Vec2f Node::calcTextSize(std::string str) {
    return this->graphics->calcTextSize(str);
}
...
void Node::setDrawGraphics(IDraw *graphics) {
