...
napi_value setChildren(napi_env env, napi_callback_info info) {
    ...
    std::shared_ptr node = utils::getNode(env, args[0]);
...
napi_value appendChildren(napi_env env, napi_callback_info info) {
    ...
    std::shared_ptr<Node> parent = utils::getNode(env, args[0]);
    std::shared_ptr<Node> children = utils::getNode(env, args[1]);

    children->parent = parent;
    if (!std::holds_alternative<holo_nodes>(parent->children)) {
        holo_nodes node;
        parent->children = node;
    }

    holo_nodes& parent_children = std::get<holo_nodes>(parent->children);

    parent_children.push_back(children);
...
napi_value init(napi_env env, napi_value exports) {
    ...
    napi_value document_appendChildren;
    ...
    napi_create_function(env, "appendChildren", NAPI_AUTO_LENGTH, appendChildren, nullptr, &document_appendChildren);
    ...
    napi_set_named_property(env, documentObj, "appendChildren", document_appendChildren);
